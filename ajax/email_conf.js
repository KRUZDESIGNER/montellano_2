...
function contactUsSubmit() {
	...
	if( name.length == 0 || name == '' || name == null || $.trim(name).length == 0 || name == 'Name *') {
		$(".form-result").css("display", "none").html("<p class='note error'>Por favor, introduzca su nombre.</p><br />").fadeIn(400);
	...
	if( !eIsValid ) {
		...
		$(".form-result").css("display", "none").html("<p class='note error'>Por favor, introduzca un E-Mail válido.</p><br />").fadeIn(400);
	...
	if( comment.length == 0 || comment == '' || comment == null ) {
		$(".form-result").css("display", "none").html("<p class='note error'>Por favor, redacte su mensaje antes de enviar.</p><br />").fadeIn(400);
		...
		if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			$(".contactForm").slideUp(1000, function() {
				...
				if( parseInt(xmlhttp.responseText) == 1 ) {
					...
					$(".form-result").css("display", "none").html("<p class='note error'>¡Lo sentimos, ha ocurrido un problema! Su mensaje no se ha podido enviar correctamente, intente de nuevo por favor. Error Message: "+xmlhttp.responseText+"</p>").fadeIn('fast');
				}
